...
const EvolutionChart = ({ data = [] }) => {
  ...
  const trend =
    delta > 0.1
      ? { Icon: TrendingUp, text: 'text-emerald-600', bg: 'bg-emerald-50', label: `+${delta} pts` }
      : delta < -0.1
      ? { Icon: TrendingDown, text: 'text-rose-600', bg: 'bg-rose-50', label: `${delta} pts` }
      : { Icon: Minus, text: 'text-slate-500', bg: 'bg-slate-100', label: 'Estable' };
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-900">Evolución del Promedio</h3>
            <p className="text-xs text-slate-500">Tendencia de tu rendimiento</p>
          </div>
        </div>
        {hasEnough && (
          <span
      ...
      {hasEnough ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center py-10 min-h-[200px]">
          ...
          <p className="text-slate-500 font-semibold text-sm mb-1">Sin datos suficientes</p>
